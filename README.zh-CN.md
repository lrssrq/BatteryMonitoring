...
- `app/(drawer)/about.tsx`：关于页
- `app/(drawer)/help.tsx`：帮助页
- `app/(drawer)/settings.tsx`：设置页
...
将 `.env.example` 复制为 `.env`，并填写所需配置。

### 3) 关键配置项

- 环境变量：`.env`（示例见 `.env.example`）
- Expo 配置注入：`app.config.ts`
- 运行时读取：`config/env.ts`
...
- 检查 `.env` 和 `app.config.ts` 中的 `API_BASE_URL`
- 确认移动设备与后端是否在同一网络
- 确认后端服务端口可访问
